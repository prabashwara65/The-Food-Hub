...
import { verifyEmail } from '../../store/driverSlice'; // Correctly import the action
...
const EmailVerification = () => {
  ...
  const sendVerificationEmail = async () => {
    try {
      const response = await axios.post('http://localhost:8000/api/send-verification-email', { email }); // Updated endpoint
  ...
  const handleVerify = async () => {
    try {
      const response = await axios.post('http://localhost:8000/api/verify-code', { email, verificationCode }); // Correct endpoint
      if (response.status === 200) {
